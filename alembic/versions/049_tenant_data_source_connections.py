...
NOTE (renumbered 2026-09-16): originally revision "040" (down_revision "039"),
independently created on this branch from `040_confidence_routed_review.py` and
`040_tenant_data_source_connections.py`'s later siblings (041-043), which
respectively claimed the same "040"-"043" ids for entirely different migrations.
Renumbered 049-052 and re-chained after the merged trunk's true head (048) —
pure bookkeeping, not a schema change; nothing here was applied to any shared
database under its old numbering.
"""
