# Graph Report - ner-project  (2026-09-11)

## Corpus Check
- 1842 files · ~1,847,340 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 21568 nodes · 33480 edges · 1351 communities (1221 shown, 38 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 1117 edges (avg confidence: 0.91)
...
- FakeSession
- test_tenant_credential_hygiene.py
- authFetch
- AnnotationPage.tsx
- extraction.ts
- entity_normalizer.py
- test_annotation_import.py
- test_ocr_media_type_resolution.py
- DocumentIngestionService
- ToolContext
- DefineEntityTypeSlideOver.tsx
- _spec
- relational_projection.py
- RetrievalResult
- FakeSession
- chat.py
...
- support.js
- vitest
- test_sql_execution_privileges.py
- DenseRetriever
- NotFoundError
...
- EntityService
- sql_generator.py
- test_entity_resolution_graph.py
- _spec
- logging_config.py
- eval/runner.py
...
- test_document_entity_provenance.py
...
- test_tracing_cross_service.py
- ContextAssembler
- test_chat_api_sql_retry_integration.py
- test_training_jobs_api.py
- training-jobs/page.tsx
...
- emit
- _get_sync_engine
- test_observability_log_export.py
- configured
...
- TestResponseCarriesRetrievalStatus
- fine_tune_model
...
- test_manual_blob_sync_trigger.py
- Requirements
- auth_header
...
- FR-001..FR-013: tenant-admin connection lifecycle, Azure Blob sync, Azure PostgreSQL direct chat, schema-contract drift blocking
...
- c4-diagram skill
- Metric contract: every metric family, its labels and closed value sets
...
- Infrastructure
- Verification Plan
- TestOneExtractionRunLeaksNothing
...
- manual-blob-sync-trigger/design.md
- _complete_run
...
- manual-blob-sync-trigger/proposal.md
- Requirement: Common durable Blob ingestion
...
- manual-blob-sync-trigger/tasks.md
...
1. `app()` - 123 edges
2. `create_access_token()` - 111 edges
3. `FakeSession` - 110 edges
4. `vitest` - 106 edges
5. `FakeLLM` - 98 edges
6. `@testing-library/react` - 94 edges
7. `authFetch()` - 93 edges
8. `RetrievalResult` - 90 edges
9. `session_factory()` - 90 edges
...
- `NAME labeling review: 367/416 resume records labeled, 49 unlabeled` --semantically_similar_to--> `Telemetry scan FAIL: 3 findings, seeded entity value leaked in log records`  [INFERRED] [semantically similar]
  scripts/name_review_report.txt → docs/observability/evidence/scan-fails-on-log-leak.txt
...
## Communities (1351 total, 38 thin omitted)

### Community 0 - "FakeSession"
Cohesion: 0.03
Nodes (79): Exception, Raised when every attempt failed. Deliberately an exception rather than a…, SQLGenerationFailed, Answers a natural-language question against extracted structured entity data.…, StructuredRetrievalTool, A statement that outruns the 10s bound is cancelled, the transaction is rolled…, verification.md rows 6, 7, 13 — the execution path either runs a validated…, definition_row() (+71 more)

### Community 1 - "test_tenant_credential_hygiene.py"
Cohesion: 0.04
Nodes (73): The three declared retention modes. Its own module, with no imports, because…, The declared set of adapter selections, and which of them are executable. A…, Names of the slots whose recorded selection is not executable in this change., unsupported_selections(), declared_secret_fields(), InvalidSecretReference, ProfileValidationError, Exception (+65 more)

### Community 2 - "authFetch"
Cohesion: 0.04
Nodes (71): @tanstack/react-query, JobsPage(), TrainingJob, NewTenantPage(), Tenant, TenantsPage(), TenantsResponse, mockFetch (+63 more)

### Community 3 - "AnnotationPage.tsx"
Cohesion: 0.03
Nodes (81): AnnotationActionBar(), AnnotationActionBarProps, SAVE_LABELS, SaveState, task, AnnotationPage(), mockAuthFetch, MY_TASK (+73 more)
...
### Community 5 - "entity_normalizer.py"
Cohesion: 0.04
Nodes (63): fold_text(), _is_adjacent(), merge_wordpieces(), Removes Unicode format characters (general category `Cf`) and folds typographic…, Whether `current` continues the entity `prev` belongs to. Model serving filters…, The entity's surface text. When the caller supplies the full ordered token…, Splits a `B-TYPE`/`I-TYPE` label into (prefix, type). Labels with no recognized…, Merges `##`-prefixed WordPiece continuation tokens into the preceding token's… (+55 more)

### Community 6 - "test_annotation_import.py"
Cohesion: 0.05
Nodes (69): compute_entity_type_counts(), generate_uuid(), get_known_entity_types_lower(), get_session(), get_tenant_id(), import_annotations(), parse_conll(), parse_jsonl() (+61 more)

### Community 7 - "test_ocr_media_type_resolution.py"
Cohesion: 0.12
Nodes (31): _media_type_from_content(), _media_type_from_declaration(), Declared type, then filename extension, then a content sniff. Never the storage…, resolve_media_type(), _counts(), asyncio, parametrize, Verification for media-type resolution — part of verification.md rows 72-81.… (+23 more)

### Community 8 - "DocumentIngestionService"
Cohesion: 0.07
Nodes (77): AST, requires_real_stores, Where this document came from, in the source's own terms. Timestamps are never…, SourceReference, InProcessDispatcher, ProcessingDispatcher, Protocol, Post-ingestion processing dispatch. The payload is document identity and tenant… (+69 more)

### Community 9 - "ToolContext"
Cohesion: 0.03
Nodes (80): ArgValidationError, assert_no_tenancy_params(), Any, Exception, Protocol, Validates `args` against a minimal JSON-Schema-shaped `args_schema` (type,…, Shared execution wrapper: validates args, times the call, and converts any…, Per-request scope handed to every tool call. Constructed by the calling… (+72 more)

### Community 10 - "DefineEntityTypeSlideOver.tsx"
Cohesion: 0.07
Nodes (29): BASE_LABELS, CARDINALITY_OPTIONS, mockFetch, cardinalityChangeMessage(), DefineEntityTypeSlideOver(), buildPayload(), handleSubmit(), sendUpdate() (+21 more)

### Community 11 - "_spec"
Cohesion: 0.06
Nodes (35): build_routing_index(), `sql_identifier -> entities routed to it`, skipping every unroutable entity. An…, The one value a `single` definition contributes to the document's `subject`…, The value a `subject` column receives, decided by the definition's…, `uppercased entity_type literal -> the one definition that claims it`. Only…, route_entities(), select_single_value(), value_for_column() (+27 more)

### Community 12 - "relational_projection.py"
Cohesion: 0.08
Nodes (29): build_projection_statements(), build_relational_delete_statements(), _checked_table(), _child_insert(), _child_params(), Projects a document's final entity list into the tenant's generated relational…, `confidence` and `page_number` are projected; provenance is not.…, Every statement that writes this document's relational rows. Executes nothing.… (+21 more)

### Community 13 - "RetrievalResult"
Cohesion: 0.05
Nodes (52): Any, resolve_rerank_candidate_count(), resolve_reranker_enabled(), resolve_top_k(), RetrievalResult, CrossEncoderReranker, _metrics(), Protocol (+44 more)
...
Cohesion: 0.05
Nodes (58): field_validator, chat(), chat_stream(), _check_tenant_and_rate_limit(), create_conversation(), delete_conversation(), get_conversation(), get_session() (+50 more)
...
Cohesion: 0.08
Nodes (52): JSONResponse, add_bearer_security(), app_error_handler(), health(), lifespan(), exception_handler, FastAPI, get (+44 more)

### Community 18 - "data-sources.ts"
Cohesion: 0.04
Nodes (88): DetailContent(), UPDATABLE, ContractsContent(), handlePublish(), handleUpload(), DataSourcesContent(), isOrder(), isProvider() (+80 more)

### Community 19 - "domain_metrics.py"
Cohesion: 0.05
Nodes (74): main(), Regenerate `docs/observability/metric-contract.md` from the live declarations.…, render_family(), _defect_class(), The defect's category, without its payload. `SQLAttempt.defect` carries the…, _record_metric(), set_trace_id(), _coerce() (+66 more)

### Community 20 - "_reconcile"
Cohesion: 0.07
Nodes (39): _column_types(), _columns(), _drop_schema(), _insert_child_row(), _make_schema(), asyncio, fixture, Integration tests for the entity table reconciler against a real tenant schema.… (+31 more)
...
### Community 22 - "vitest"
Cohesion: 0.03
Nodes (80): react-dom, @testing-library/react, vitest, SettingsPage(), DocumentRow(), DocumentRowProps, formatDate(), purposeLabel() (+72 more)

### Community 23 - "test_sql_execution_privileges.py"
Cohesion: 0.08
Nodes (32): _async_dsn(), main(), Provision the least-privilege role that generated chat SQL executes under, then…, build_role_statements(), _checked_identifier(), InvalidIdentifierError, list_tenant_schemas(), provision_role() (+24 more)

### Community 24 - "DenseRetriever"
Cohesion: 0.05
Nodes (62): _fake_vector(), FakeEmbeddingService, main(), Manual playground for HybridRetriever/SparseRetriever/DenseRetriever — no…, Returns a fixed query vector — swap for the real EmbeddingService if you have…, run_query(), setup(), DenseRetriever (+54 more)

### Community 25 - "NotFoundError"
Cohesion: 0.05
Nodes (41): create_tenant(), create_tenant_user(), CreateTenantRequest, deactivate_tenant(), get_tenant(), list_audit_log(), list_tenant_users(), list_tenants() (+33 more)

### Community 26 - "@testing-library/user-event"
Cohesion: 0.04
Nodes (35): @testing-library/user-event, AuditEventRow(), AuditPage(), formatTimestamp(), KIND_COLORS, mockEvents, mockRefetch, mockTenants (+27 more)
...
### Community 30 - "ingestion/service.py"
Cohesion: 0.04
Nodes (51): requires_minio, retry, ContentStore, Protocol, StorageReference, The application-owned boundary for a document's bytes. Three operations and…, Store bytes and return the reference by which they can be reopened. `filename`…, Return the bytes at a previously returned reference, or None if they are gone. (+43 more)

### Community 31 - "SQLGenerator"
Cohesion: 0.04
Nodes (44): _error_class(), _metrics(), AsyncSession, The role generated SQL runs under, or None to keep the connection role. Read…, Executes an already-validated statement and returns its rows. When…, The total the statement would have matched without its row limit, or None if…, Bounded, representative values per stored `entity_type`, still read from the…, One relation, or one `subject` entity column, as the generator is told about… (+36 more)

### Community 32 - "EntityService"
Cohesion: 0.06
Nodes (49): create_entity_type(), delete_entity_type(), EntityTypeCreate, EntityTypeToggle, EntityTypeUpdate, get_entity_type(), list_entity_types(), AsyncSession (+41 more)

### Community 33 - "sql_generator.py"
Cohesion: 0.05
Nodes (63): accepted_columns(), accepted_relations(), _fix_document_name_reference(), _force_nulls_last_on_desc(), _is_identifier_token(), iter_table_references(), _langsmith_extra(), _parse_table_list() (+55 more)

### Community 34 - "test_entity_resolution_graph.py"
Cohesion: 0.06
Nodes (26): setter, Returns a new RetrievalPlan with every `semantic_retrieval` entry's `scope`…, _rewrite_plan_for_resolution(), OrchestrationResult, _base_state(), nodes(), fixture, Covers verification.md rows 14, 32, 33, 34. (+18 more)

### Community 35 - "_spec"
Cohesion: 0.06
Nodes (31): build_child_table_statements(), build_entity_table_statements(), build_subject_table_statements(), entity_type_literals(), Deterministic, collision-free, <=63 chars, matches `^e_[a-z0-9][a-z0-9_]*$`.…, Every stored `entity_type` value that means this definition, uppercased and…, `CREATE TABLE IF NOT EXISTS` plus its `normalized_value` index, for one `multi`…, The `subject` table, then one `ADD COLUMN IF NOT EXISTS` per active `single`… (+23 more)

### Community 36 - "logging_config.py"
Cohesion: 0.05
Nodes (55): Formatter, Handler, LogRecord, current_context(), get_tenant_id(), get_trace_id(), get_user_hash(), Ambient request context for telemetry. Threading… (+47 more)
...
Cohesion: 0.08
Nodes (30): AzureBlobLiveProvider, SDK-backed Azure Blob provider (CAP-3, ADR-012). Replaces…, One tenant connection's live Azure Blob container, via the SDK., Durable Azure Blob synchronization runtime (CAP-3, ADR-012). One tenant-bound…, BlobListingFailed, BlobObject, BlobObjectMissing, BlobProvider (+22 more)

### Community 39 - "SQLValidationError"
Cohesion: 0.05
Nodes (25): SQLValidationError, asyncio, Covers verification.md rows 34-37 — offline, no DB required., Covers verification.md rows 19, 22-25 — deterministic filtering over real rows., TestStructuredValueSQLExecution, TestStructuredValueSQLValidation, The whitelist check used to resolve only the first identifier after each…, `public.documents` is not `documents` — the qualifier is grounds for rejection,… (+17 more)

### Community 40 - "extraction_service/main.py"
Cohesion: 0.07
Nodes (56): AsyncEngine, add_bearer_security(), app_error_handler(), health(), health_live(), lifespan(), exception_handler, FastAPI (+48 more)

### Community 41 - "Source"
Cohesion: 0.04
Nodes (36): Queue, CandidateEntity, ChatResponse, Citation, PendingClarification, Source, _candidates_to_schema(), Candidate (+28 more)

### Community 42 - "_patch"
Cohesion: 0.08
Nodes (43): _proxy(), proxy_chat(), proxy_chat_stream(), proxy_create_conversation(), proxy_create_widget_key(), proxy_delete_conversation(), proxy_get_conversation(), proxy_list_conversations() (+35 more)
...
Cohesion: 0.14
Nodes (28): _get_tenant_id(), _get_user_id(), list_entities(), patch_entity(), AsyncSession, get, Request, EntityItem (+20 more)

### Community 45 - "test_external_postgresql_chat.py"
Cohesion: 0.16
Nodes (21): is_external_request_executable(), Server-side external-query capability resolution (CAP-4, ADR-001/011/013).…, Resolve the tenant's executable external-query capability. Returns…, Whether one named connection may serve this tenant's external request., resolve_external_capability(), accepted_contract(), list_versions(), The tenant's published contract for one connection, or None. (+13 more)

### Community 46 - "sync.py"
Cohesion: 0.06
Nodes (54): delete_document(), get_document(), get_document_text(), get_session(), get_tenant_id(), list_documents(), AsyncSession, delete (+46 more)
...
### Community 50 - "get_active_model"
Cohesion: 0.08
Nodes (33): _cache_model_version(), demote_model_version(), _experiment_name(), get_active_model(), _get_client(), list_model_versions(), _lookup_run_number(), _metrics_with_label_list() (+25 more)
...
Cohesion: 0.10
Nodes (50): _activity_tag_colour(), ActivityRow, _all_tenant_schemas(), _annotator_continue_work(), _annotator_data(), _annotator_side_panel(), _annotator_task_activity(), _annotator_type_counts() (+42 more)

### Community 53 - "test_document_entity_provenance.py"
Cohesion: 0.12
Nodes (18): _entity(), asyncio, fixture, Covers verification.md rows 56-63. `document_entities` could not previously…, Row 57 — a NULL means unchanged, not unknown., Rows 59-62 at the storage boundary., Row 60 — a quality shift must be traceable to a prompt revision., Row 62 — a logit cannot be converted to a probability after the fact. (+10 more)

### Community 54 - "test_azure_blob_source_sync.py"
Cohesion: 0.10
Nodes (66): acquire_lease(), confirm_missing(), ensure_sync_tables(), get_source(), hidden_document_ids(), hide_document(), last_successful_run_at(), ledger_identities() (+58 more)
...
### Community 56 - "nodes.py"
Cohesion: 0.04
Nodes (60): _DummyOrchestrator, main(), Generates a Mermaid diagram for the chat graph's single, fixed topology using…, build_nodes() only closes over this; node bodies never run here., build_chat_graph(), Compiles the chat graph topology. With `entity_resolution_enabled` off (the…, _route_after_entity_resolution(), _route_after_guardrail() (+52 more)

### Community 57 - "DocumentTable.tsx"
Cohesion: 0.07
Nodes (24): DocumentsPage(), DocumentTable(), DocumentTableProps, baseDoc, qc, ACCEPTED_TYPES, BatchItem, DocumentUpload() (+16 more)
...
### Community 59 - "Base"
Cohesion: 0.06
Nodes (32): DeclarativeBase, AuditEvent, Base, EntityDefinition, str, Tenant, TenantStatus, TenantUser (+24 more)

### Community 60 - "react"
Cohesion: 0.03
Nodes (67): lucide-react, react, react-markdown, remark-gfm, ChatPage(), ChatPageInner(), Conversation, Message (+59 more)

### Community 61 - "app"
Cohesion: 0.06
Nodes (30): app(), fixture, auth_header(), TestAnalyticsQueryEndpoint, auth_header(), asyncio, TestCorrectEntity, TestCorrectEntityAnnotator (+22 more)
...
Cohesion: 0.11
Nodes (16): filter_valid_entities(), is_valid_entity(), Whether an entity is a fact worth storing. `NOT NULL` does not catch an empty…, Partitions entities into those worth persisting and a count of those dropped.…, _entity(), parametrize, Covers verification.md rows 20-23. `document_entities.normalized_value` is `NOT…, The gate must not become a second source of missing entities. (+8 more)
...
### Community 66 - "_Setter"
Cohesion: 0.25
Nodes (4): BaseException, Attribute setter that is a no-op when there is no live span., The exception's *class*, not its message. A driver message quotes the offending…, _Setter

### Community 67 - "tracked_tenants"
Cohesion: 0.11
Nodes (17): action_headers(), admin_headers(), create_draft(), Verification for the tenant-scoped connection control plane (CAP-2). Maps to…, Tenants this test creates, removed with their control-plane rows after., Row 11: the Azure-bounded lifecycle grants no platform-profile access., Row 9: an activated Azure capability resolves for its tenant only., Row 10: inactive/unsupported sources never alter the executable set. (+9 more)

### Community 68 - "NormalizedEntity"
Cohesion: 0.10
Nodes (42): NormalizedEntity, apply_decisions(), build_candidates(), _build_client(), build_window(), call_postprocessor(), Candidate, _comparable() (+34 more)

### Community 69 - "canonicalize"
Cohesion: 0.07
Nodes (15): canonicalize(), Deterministic fallback (format-character removal, typographic folding, NFKC,…, verification.md rows 10-13, TestCanonicalNormalization, asyncio, Covers verification.md rows 8-11. The defect: `canonicalize()` applied NFKC,…, The evidence check in post-processing folds without casefolding, so the helper…, Row 8's second clause: the point of folding is that the persisted row is… (+7 more)
...
### Community 71 - "users/page.tsx"
Cohesion: 0.07
Nodes (27): Favicon — dark theme SVG, Logo — dark theme SVG, Hardcoded-color-to-CSS-variable mapping (text-gray-900 -> var(--ink), bg-white -> var(--surface-2), etc.), Design: Fix Dark Theme Issues Across Portal Pages, Proposal: Fix Dark Theme Issues Across Portal Pages, Fix Dark Theme Issues - Tasks, In-app Logo SVG, Logo (Dark Theme) SVG (+19 more)

### Community 72 - "shared/config.py"
Cohesion: 0.03
Nodes (43): Sync tenant_{id} schemas with tenant_template. Clones missing tables and adds…, _init_worker_observability(), connect, Wire this worker's telemetry when it starts as a worker. Bound to the signal…, Ensure the ner_mlflow database exists before the MLflow tracking server starts.…, _retrying(), Opaque user identity for telemetry. Support needs to follow one person's…, init_observability() (+35 more)

### Community 73 - "_get"
Cohesion: 0.10
Nodes (18): _platform_health_status(), auth_header(), _get(), asyncio, _seed_annotator_tasks(), _seed_spans(), test_platform_health_status_critical_when_gateway_or_model_serving_offline(), test_platform_health_status_degraded_when_noncritical_offline() (+10 more)
...
### Community 75 - "external_postgres/contract.py"
Cohesion: 0.12
Nodes (20): canonical_fingerprint(), CanonicalContract, ContractValidation, fingerprint_of_metadata(), _is_safe_name(), Canonical schema-contract lifecycle (CAP-4, ADR-013). A contract declares…, Deterministic fingerprint over sorted relations/columns/normalized joins., Fingerprint over live introspected metadata using the same canonical form. (+12 more)
...
### Community 82 - "test_tracing_cross_service.py"
Cohesion: 0.14
Nodes (12): _get(), fixture, One unit of work, one trace identifier, across processes — and on its log…, Row 20 — a record's `trace_id` equals the trace id of the spans beside it., A null here would make every log-to-trace link in Grafana dead, while every…, Row 19. The cross-process half needs the running stack and a real broker; see…, Two instrumented services and the spans both produce. A single in-memory…, _records() (+4 more)

### Community 83 - "ContextAssembler"
Cohesion: 0.05
Nodes (48): AdmittedEvidence, build_system_prompt(), collapse_duplicate_rows(), ContextAssembler, _count_tokens(), _dedupe_chunks(), _label_for(), Returns rendered chunk text (label + body), overlap-trimmed. Never mutates… (+40 more)

### Community 84 - "test_chat_api_sql_retry_integration.py"
Cohesion: 0.09
Nodes (27): _answer_from(), _AnswerLLM, _orchestrator(), Bounded SQL recovery through the retrieval and answer-generation stages —…, Row 24 — an honest refusal, not a confident empty answer., Row 28 — rejected before execution, retried within budget, reported., Row 24's counterpart — an empty answer must not look like a failure., Row 31 — recovery is invisible to the answer stage. (+19 more)
...
### Community 86 - "training-jobs/page.tsx"
Cohesion: 0.09
Nodes (26): ViewMode, BASE_MODEL_ID, BASE_MODEL_NAME, BaseModelCard(), BaseModelCardProps, BaseModelPanel(), CONLL_LABELS, FilterTab (+18 more)
...
### Community 90 - "ocr_worker.py"
Cohesion: 0.04
Nodes (49): needs_tesseract, RuntimeError, skip, EmbeddingService, _embed_chunks(), extract_text_doc(), extract_text_image(), extract_text_pdf() (+41 more)
...
### Community 94 - "v1/auth.py"
Cohesion: 0.27
Nodes (10): Response, login(), logout(), AsyncSession, post, Request, refresh(), _set_refresh_cookie() (+2 more)

### Community 95 - "AnalyticsQueryRequest"
Cohesion: 0.18
Nodes (15): analytics_export(), analytics_query(), analytics_refresh(), AsyncSession, post, Request, AnalyticsQueryRequest, get_db() (+7 more)

### Community 96 - "extraction.py"
Cohesion: 0.14
Nodes (34): extract_entities(), get_batch_status(), _get_role(), _get_tenant_id(), list_batch_runs(), list_eligible_documents(), AsyncSession, get (+26 more)
...
Cohesion: 0.05
Nodes (69): langsmith_extra(), One hop between a LangSmith run and the OTel trace it happened inside. They are…, `langsmith_extra=` for one wrapped provider call, or `{}` when unavailable.…, _run_id_setter(), current_trace_id(), Span helpers for the workload instrumentation. Auto-instrumentation gives one…, The ambient trace id as a 32-character hex string, or None outside a trace., A span for one stage, yielding a setter for attributes decided inside the… (+61 more)

### Community 99 - "tracing.py"
Cohesion: 0.09
Nodes (21): build_tracer_provider(), init_tracing(), instrument_app(), OpenTelemetry tracing, wired once per process. Spans come from auto-…, Register the auto-instrumentors, each independently. One instrumentor failing —…, Construct this process's provider, with an exporter only if one is configured.…, Install the tracer provider and register auto-instrumentation. Idempotent: a…, Attach FastAPI instrumentation to one application. Separate from `init_tracing`… (+13 more)

### Community 100 - "training_jobs.py"
Cohesion: 0.20
Nodes (28): ApproveJobRequest, ModelVersionPromoteRequest, BaseModel, RejectJobRequest, TrainingJobCreate, TrainingJobListResponse, TrainingJobResponse, _all_active_tenant_ids() (+20 more)
...
### Community 105 - "training_service/worker.py"
Cohesion: 0.10
Nodes (16): _extract_label_set(), _load_annotated_dataset(), _make_service_token(), Exception, tokenize_and_align_labels(), TrainingDataError, slow, TestAnnotationServiceURL (+8 more)
...
Cohesion: 0.14
Nodes (28): _compute_bio_tags(), create_span(), delete_span(), generate_uuid(), get_session(), get_tenant_id(), list_spans(), prelabel_document() (+20 more)

### Community 111 - "test_entity_resolver_mentions.py"
Cohesion: 0.08
Nodes (18): _depossessive(), _mention_matches(), Whether a mention identifies the person whose stored name is…, Strips a trailing possessive from a canonical mention, or None if there is…, _coro(), _FakeSession, asyncio, parametrize (+10 more)

### Community 112 - "inference_service.py"
Cohesion: 0.14
Nodes (23): _get_base_pipeline(), _get_tokenizer(), infer(), _infer_window(), _infer_with_base_model(), _infer_with_onnx(), _load_model_for_tenant(), WordPiece length of each whitespace word, in word order. Uses one… (+15 more)
...
### Community 114 - "_get_sync_engine"
Cohesion: 0.16
Nodes (11): _get_sync_engine(), Write job fields, and count a state transition when the status is one of them.…, _schema(), _update_job_progress(), _cleanup_test_schema(), _create_test_schema(), Tests for training worker functions: _update_job_progress and model_versions…, Mirrors the run_number lookup/insert worker.py performs on job completion (see… (+3 more)

### Community 115 - "test_observability_log_export.py"
Cohesion: 0.09
Nodes (22): Restore the value this context had before `set_request_id`. Resetting rather…, reset_request_id(), _attributes(), _bodies(), exported(), fixture, Log records leave the process, not just the container's stdout. Verification…, Row 32 — delivery, not emission. (+14 more)

### Community 116 - "configured"
Cohesion: 0.11
Nodes (17): configured(), fixture, parametrize, Deriving it from the message would make the client IP the event name — a…, The reason this matters: an access line is the one record that quotes a caller-…, Row 3 — `NER_LOG_LEVEL` governs what is emitted., Row 5 — present, and null, when no request is active., `configure_logging` calls `basicConfig(force=True)`, which detaches every root… (+9 more)

### Community 117 - "test_mlflow_verification.py"
Cohesion: 0.10
Nodes (18): MlflowClient, _get_sync_engine(), _read_cache_active_model(), _read_cache_model_versions(), _schema(), mlflow_client(), cleanup(), db_schema() (+10 more)
...
### Community 138 - "entity_resolver.py"
Cohesion: 0.04
Nodes (57): _candidates_from_json(), _candidates_to_json(), clear_binding(), clear_pending(), ConversationState, decode_document_ids(), encode_document_ids(), increment_reask() (+49 more)
...
### Community 144 - "test_conversation_history_reaches_retrieval.py"
Cohesion: 0.11
...
Cohesion: 0.05
Nodes (37): aggregate_confidence(), Reconstructs complete logical entities from an ordered, WordPiece-merged…, Entity-level confidence is the minimum of its constituent tokens' confidences —…, reconstruct_entities(), _pred(), `reconstruct_entities` used to continue an entity across an `I-` tag no matter…, The observed bug: an I-INSTITUTION eleven words later got glued on., The base-model pipeline path emits WordPieces with no word alignment, so there… (+29 more)
...
Cohesion: 0.16
Nodes (22): get_tenant_id(), inference_endpoint(), post, Request, get_tenant_id(), post, Request, rerank_endpoint() (+14 more)
...
### Community 157 - "annotation_service/main.py"
Cohesion: 0.11
Nodes (25): export_annotations(), get_session(), get_tenant_id(), AsyncSession, get, Request, _tokenize(), create_task() (+17 more)

### Community 158 - "extraction_service/worker.py"
Cohesion: 0.06
Nodes (48): backfill_document(), backfill_semantic_values_for_document(), _documents_for_semantic_backfill(), _documents_needing_backfill(), _fetch_token_records(), main(), Backfill `document_entities` for documents extracted before normalized entity…, Populates typed value columns for an already-normalized document by re-parsing… (+40 more)

### Community 159 - "imported-documents/page.tsx"
Cohesion: 0.05
Nodes (48): mockAuthFetch, mockUseAnnotationImport, mockUseAuth, mockUseEntityTypes, buildEntityColors(), ENTITY_COLORS, ImportedDocumentReview(), ImportedDocumentsList() (+40 more)
...
Cohesion: 0.13
Nodes (21): _convert_duration(), _digits_to_number(), _extract_open_bound_phrase(), _extract_range(), _extract_source_unit(), _leading_numeral_to_number(), _parse_boolean_kind(), _parse_date_kind() (+13 more)
...
Cohesion: 0.18
Nodes (11): _get(), BaseHTTPMiddleware, A record emitted during a request carries that request's context, opaquely.…, A leaked contextvar would silently merge two users' work under one identifier., Rows 15 and 17 — a hash for the user, a UUID for the tenant, nothing else., Stands in for each service's real `TenantContextMiddleware`. Only the part this…, Row 4 — the four fields match the values resolved for that request., _records() (+3 more)

### Community 169 - "TestAMalformedSchemaIsCountedAndLogged"
Cohesion: 0.13
...
Cohesion: 0.21
Nodes (9): _entity(), fixture, Covers verification.md rows 53-55. ADR-001 requires zero cross-tenant leakage,…, Tenant scope is the worker's, resolved from server-controlled context; the…, stable_settings(), TestEvidenceWindowIsBounded, TestOneDocumentPerRequest, TestOutputCannotRedirectPersistence (+1 more)
...
### Community 172 - "test_relational_document_delete.py"
Cohesion: 0.23
Nodes (10): _count(), _define(), Deleting a document clears its rows from every generated relational table. The…, verification.md rows 89, 90, 91, 92, verification.md row 29 — the property that keeps the two paths from diverging., _reconcile(), _seed(), _spec() (+2 more)
...
### Community 178 - "assert_tenant_schema"
Cohesion: 0.21
Nodes (12): handle_extraction_completed(), task, refresh_analytics_materialized_views(), assert_tenant_schema(), Check a schema name against the tenant-schema pattern before it is…, sanitize_schema_for_log(), get_current_role(), get_current_user_id() (+4 more)

### Community 179 - "test_entity_resolution_metrics.py"
Cohesion: 0.15
...
Cohesion: 0.08
Nodes (60): _require_keys(), Tenant-scoped Azure connection control plane (CAP-2, ADR-011)., LifecycleRejected, Exception, Finite lifecycle, test, and activation evidence classes (CAP-2). Every value…, A lifecycle action the connection's state, evidence, or limits forbid. Carries…, ConnectionValidationError, Exception (+52 more)
...
### Community 186 - "resolve_query_surface"
Cohesion: 0.11
Nodes (20): `schema -> QuerySurface` for the execution role, `validate_sql`, and the…, The tenant schema name for a `entity_definitions.tenant_id`. One direction…, resolve_query_surface(), schema_for_tenant(), Answers `_QUERY_SURFACE_QUERY` from canned `public.entity_definitions` rows.…, verification.md rows 66-71 — the one authoritative description of the readable…, Rows 66, 68 — identifiers alone are not enough; a consumer needs the types., Row 68 — without `value_kind` on the row every column resolves TEXT, and a… (+12 more)
...
### Community 190 - "TestResponseCarriesRetrievalStatus"
Cohesion: 0.21
Nodes (6): _app(), auth_header(), CannedOrchestrator, _fake_citation(), A client reading only the previously specified fields sees no change, and a…, TestResponseCarriesRetrievalStatus

### Community 191 - "fine_tune_model"
Cohesion: 0.22
...
### Community 198 - "test_manual_blob_sync_trigger.py"
Cohesion: 0.16
Nodes (25): evaluate_connection(), datetime, Sync scheduling evaluation (CAP-3, ADR-012). The beat scheduler evaluates every…, Decide whether a connection is due for a scheduled run or a catch-up., ScheduleDecision, active_connection(), enqueued(), fixture (+17 more)
...
### Community 206 - "TestDashboardSummaryShape"
Cohesion: 0.18
Nodes (5): DashboardSummaryResponse, get_dashboard_summary(), get, Request, TestDashboardSummaryShape
...
### Community 220 - "model_serving/main.py"
Cohesion: 0.19
Nodes (11): add_bearer_security(), app_error_handler(), health(), lifespan(), exception_handler, FastAPI, get, Request (+3 more)
...
Cohesion: 0.05
Nodes (38): results, @axe-core/playwright, eslint, eslint-config-next, eslint-config-prettier, jsdom, next, @playwright/test (+30 more)
...
### Community 232 - "build_subject_column_type_statements"
Cohesion: 0.11
Nodes (15): build_subject_column_type_statements(), diverging_subject_columns(), `(column, actual type, declared type)` for every `subject` column that…, One `ALTER COLUMN ... TYPE` per diverging `subject` column, clearing what it…, `ADD COLUMN IF NOT EXISTS` does nothing to a column that already exists, so a…, Row 1 — the `PHONE_NUMBER` shape: catalog says number, disk says text., Rows 1-3 — including the pair PostgreSQL provides no cast between., Row 5 — and the reason Risk 1 exists: a spelling mismatch here would retype,… (+7 more)
...
### Community 236 - "TestModelQualityIsNotMirrored"
Cohesion: 0.27
...
### Community 239 - "FR-001..FR-013: tenant-admin connection lifecycle, Azure Blob sync, Azure PostgreSQL direct chat, schema-contract drift blocking"
Cohesion: 0.33
Nodes (6): Ralph run report: CAP-2 tenant-scoped connection control plane blocked, architecture-resolver returned ESCALATE, CAP-2..CAP-6 dependency chain: CAP-3/4 blocked by CAP-2, CAP-5/6 blocked by CAP-3, Azure Blob Storage as synchronized document source, Azure Database for PostgreSQL direct read-only structured-data chat, FR-001..FR-013: tenant-admin connection lifecycle, Azure Blob sync, Azure PostgreSQL direct chat, schema-contract drift blocking, Versioned JSON schema-contract with tenant-isolated vector index and drift check
...
### Community 241 - "TestBackfillSemanticValues"
Cohesion: 0.39
...
Cohesion: 0.25
Nodes (8): Current-state platform inventory: 22 compose containers, config surface, portal URL wiring, Decision Register DEC-001..DEC-019: broker, LLM provider, GPU, registry, ingress, secrets, observability backends, DR targets, Deploy Platform to Kubernetes on Azure (AKS) — requirements baseline, F-1..F-7 documented-vs-implemented divergences (observability reversal, broker contradiction, topology drift, RPO/RTO drift, hostname drift), FR-001..FR-014: deploy 8 services + 2 workers + portal + MLflow to AKS with probes, secrets, TLS, CI, GPU training, backups, Risk Register R-001..R-016: 3-developer team overload, in-process rate limiter, self-hosted Postgres HA, Requirements — Multi-Tenant Custom Named Entity Recognition Platform (approved 2026-06-03), Project Requirements Document v0.2 — Multi-Tenant Custom NER Platform (PRD draft)
...
### Community 251 - "connector.py"
Cohesion: 0.15
Nodes (20): Identifier, External PostgreSQL chat capability (CAP-4, ADR-013). A separate selection from…, Drift-gated read-only external query execution (CAP-4, ADR-013). Order per…, _record(), _check_columns(), _first_keyword(), _function_names(), _identifiers_in_from() (+12 more)
...
### Community 256 - "execute_external_query"
Cohesion: 0.20
Nodes (19): clamp_limit(), execute_external_query(), Enforce the server row cap: keep a smaller LIMIT, else append one., Run one drift-gated, validated, parameterized external SELECT. Returns…, FixtureExternalDatabase, In-memory fake: declared metadata plus scripted rows, no network., _make_connection(), _publish() (+11 more)
...
### Community 262 - "session_factory"
Cohesion: 0.17
Nodes (38): activate_profile(), Activate a validated profile, unless it selects something not yet executable., Record a profile. Validation is by declared schema, never by value inspection.…, write_profile(), load_profile(), Read a tenant's profile, falling back to the platform defaults. A missing table…, session_factory(), Row 7: a recorded-but-unsupported value succeeds as a write, stays draft. (+30 more)
...
Cohesion: 0.14
Nodes (35): enqueue_sync(), activate_data_source(), _body_digest(), create_data_source(), _created(), _enqueue_blob_sync(), _error(), _idempotency_key() (+27 more)
...
Cohesion: 0.07
Nodes (19): _count_rejection(), Count one rejection, splitting the bucket key into its scope and tenant. Keys…, SlidingWindowRateLimiter, A validated token asserting one tenant, addressing another. The counter and the…, TenantMismatchError, record_rate_limit_rejection(), The four counters Exit Gate 3 names: mismatch, auth failure, rate-limit…, Row 32's second clause. The reason is drawn from the exception type, so there… (+11 more)

### Community 454 - "test_orchestrator_integration.py"
Cohesion: 0.17
...
### Community 526 - "AnalyticsExportRequest"
Cohesion: 0.21
Nodes (5): AnalyticsExportRequest, auth_header(), asyncio, TestAnalyticsExportEndpoint, TestExportSchemaValidation

### Community 527 - "resolver.py"
Cohesion: 0.32
Nodes (7): active_connection(), executable_providers(), is_selection_executable(), Server-side tenant capability resolution for approved Azure connections…, The tenant's active connection of one approved provider, or None. Unknown…, Approved providers the tenant may execute right now, sorted., Whether one recorded profile selection may serve a request. Platform defaults…
...
### Community 542 - "load_entity_definition_specs"
Cohesion: 0.18
Nodes (15): _as_mapping(), load_entity_definition_specs(), The same catalog, as the value object the relational layer needs. A sibling of…, `base_label_mapping` reads back as a dict on JSONB and as text on a TEXT column., catalog(), _insert(), fixture, `load_entity_definition_specs` — the catalog read the relational projection… (+7 more)

### Community 543 - "classify_processing_error"
Cohesion: 0.12
Nodes (16): classify_processing_error(), ContentUnresolvable, BaseException, Exception, ValueError, The document's bytes cannot be obtained under its recorded retention mode., `source_only` resolution needs a reopenable source adapter. None exists yet;…, The resolved media type has no extraction path. A ValueError subclass so… (+8 more)
...
### Community 796 - "Requirement: Safe activation and concurrent capability limits"
Cohesion: 0.17
Nodes (11): Purpose, Requirement: Safe activation and concurrent capability limits, Requirement: Tenant-admin-managed finite Azure connections, Requirements, Scenario: Cross-tenant connection access is denied, Scenario: Duplicate active provider is rejected, Scenario: Failed prerequisite blocks activation, Scenario: Independent approved connections activate concurrently (+3 more)
...
### Community 799 - "SecureTestResult"
Cohesion: 0.14
Nodes (13): _endpoint_for(), The finite result of one secure connection test., The shipped tester: TCP connect plus TLS handshake, credentials never sent. A…, SecureTestResult, TlsHandshakeTester, client(), _env_refs(), fake_testers() (+5 more)

### Community 800 - "_LLMCall"
Cohesion: 0.13
Nodes (10): estimate_llm_cost_usd(), Label, _LLMCall, Spend for one call at the configured rates, or 0.0 when none are configured., `tenant_id` is the one label whose value set is genuinely open. It is…, Accumulator for one measured provider call., Read token counts off a provider response, tolerating one that has none. A…, One label key and the closed set of values it may carry. `values` is required… (+2 more)
...
### Community 859 - "Requirement: Data source collection and navigation"
Cohesion: 0.13
Nodes (14): Purpose, Requirement: Data source collection and navigation, Requirement: Safe connection lifecycle interface, Requirement: Schema-contract administration interface, Requirements, Scenario: Activation is blocked safely, Scenario: Administrator filters data sources, Scenario: Collection uses server-backed paging defaults (+6 more)
...
### Community 902 - "ExternalDatabase"
Cohesion: 0.17
Nodes (9): external_chat_answer(), ExternalNotExecutable, Exception, Answer one external chat turn: resolve, then drift-gated execution. Returns…, No executable external capability for this tenant; finite reason only., ExternalDatabase, Seam for live external metadata + execution. Production implements…, Return {relation: [columns]} for the named relations. (+1 more)

### Community 903 - "blob_sync/tasks.py"
Cohesion: 0.18
Nodes (9): blob_sync_tick(), task, Celery entry points for durable Blob sync (CAP-3, ADR-006). Follows the…, Worker entry point. Returns the finite run outcome for the result backend., Runs every `TICK_INTERVAL_SECONDS` on the beat schedule. Cheap by design: the…, run_blob_sync_task(), parametrize, Blob sync Celery tasks route to the queue the blob_sync worker consumes.… (+1 more)
...
### Community 956 - "gateway/main.py"
Cohesion: 0.17
Nodes (17): _proxy(), proxy_analytics_dashboard(), proxy_analytics_export(), proxy_analytics_query(), proxy_analytics_refresh(), get, post, Request (+9 more)
...
### Community 1170 - "Verification Plan"
Cohesion: 0.15
Nodes (12): 1. Spec Alignment, 2. Hallucination Risk Register, 3. Pattern & ADR Compliance, 4. Evidence Requirements, 5. Evidence Log, 6. Audit Record, AI Output Review, Edge Case Evidence (+4 more)

### Community 1171 - "TestOneExtractionRunLeaksNothing"
Cohesion: 0.38
Nodes (3): parametrize, Without this the three assertions above pass against nothing at all., TestOneExtractionRunLeaksNothing
...
Cohesion: 0.20
Nodes (9): Bugs: Tenant Self-Service Data Sources, DS-001: Last-run status never updates, DS-002: Portal shows `INTERNAL_ERROR` for every data-source error, DS-003: Blob sync scheduler appears stuck, DS-004: Gateway blocks its event loop while enqueuing a manual sync, DS-005: Gateway had no Celery broker configured, Current-state brownfield seams: upload flow, ingestion/retention, control plane, chat, durability gap, Decision Register DEC-001..DEC-006: drift detection mechanism, connection network/identity model, legal-hold approval (+1 more)
...
### Community 1256 - "entity_views.py"
Cohesion: 0.06
Nodes (44): _active(), _as_list(), _as_mapping(), catalogued_table_names(), child_index_name(), expected_table_names(), _generated_names(), generated_table_names() (+36 more)
...
### Community 1288 - "manual-blob-sync-trigger/design.md"
Cohesion: 0.17
...
### Community 1289 - "_complete_run"
Cohesion: 0.29
Nodes (5): _complete_run(), _create_run(), TestModelRegistration, TestModelRegistryProxy, TestTenantIsolation
...
### Community 1313 - "Requirement: Manual Blob sync trigger action"
Cohesion: 0.22
...
### Community 1315 - "create_widget_key"
Cohesion: 0.32
Nodes (8): create_widget_key(), list_widget_keys(), AsyncSession, delete, get, post, Request, revoke_widget_key()
...
### Community 1322 - "manual-blob-sync-trigger/proposal.md"
Cohesion: 0.25
Nodes (7): Capabilities, Impact, Modified Capabilities, New Capabilities, Open Questions, What Changes, Why

### Community 1323 - "Requirement: Common durable Blob ingestion"
Cohesion: 0.25
...
### Community 1324 - "_filename_filter_literals"
Cohesion: 0.43
...
### Community 1325 - "037_entity_definitions_view_metadata.py"
Cohesion: 0.40
Nodes (4): _backfill_sql_identifiers(), view-layer metadata on entity_definitions The read model for extracted entities…, Assigns every existing row an identifier, resolving collisions within each…, upgrade()

### Community 1326 - "allowlist_violations"
Cohesion: 0.40
Nodes (5): allowlist_violations(), families_carrying_tenant_label(), Every declared family whose label set includes `tenant_id`., Declared families carrying `tenant_id` that the allowlist does not name., test_blob_sync_families_carry_no_tenant_label()
...
### Community 1329 - "Requirement: Manual sync-now control"
Cohesion: 0.29
...
### Community 1330 - "ExternalExecutionFailed"
Cohesion: 0.50
Nodes (3): ExternalExecutionFailed, Exception, The external database failed; carries the error class only.
...
### Community 1340 - "manual-blob-sync-trigger/tasks.md"
Cohesion: 0.40
...
### Community 1344 - "EngineResolver"
Cohesion: 0.40
Nodes (4): EngineResolver, The single place an engine is chosen. Today there is exactly one engine for…, Replace the resolver. For tests and for the later data-plane change., set_engine_resolver()

### Community 1345 - "_get"
Cohesion: 0.50
Nodes (3): _get(), The point is that it is accepted *as a correlation value* — a test that passed…, TestTheHeaderDoesNotInfluenceIdentity

### Community 1346 - "TestModeDoesNotAffectSkipLogic"
Cohesion: 0.40
...
## Knowledge Gaps
- **9079 isolated node(s):** `$schema`, `plugin`, `vendorRoot`, `SessionMessage`, `SessionMessageInfo` (+9074 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 12120 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
