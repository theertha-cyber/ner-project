...
export default function TenantsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    apiFetch<TenantsResponse>(`/api/v1/admin/tenants?page=${page}&per_page=20`)
      .then(setData)
              ...
              <tbody className="divide-y divide-gray-200">
                {data.tenants.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{t.max_users}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{new Date(t.created_at).toLocaleDateString()}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                      <Link href={`/admin/tenants/${t.id}`} className="text-blue-600 hover:text-blue-900">
                        View
                      </Link>
                    </td>
                  </tr>
