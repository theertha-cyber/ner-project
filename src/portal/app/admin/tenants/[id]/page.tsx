...
export default function TenantDetailPage({ params }: { params: { id: string } }) {
  ...
  const [form, setForm] = useState({ max_users: 0, max_documents: 0, max_storage_gb: 0, max_model_versions: 0 });
  const router = useRouter();

  useEffect(() => {
    apiFetch<{ tenant: TenantDetail }>(`/api/v1/admin/tenants/${params.id}`)
      .then((data) => {
  ...
  const handleUpdate = async () => {
    try {
      const data = await apiFetch<{ tenant: TenantDetail }>(`/api/v1/admin/tenants/${params.id}`, {
        method: "PUT",
        body: form,
      });
      setTenant(data.tenant);
  ...
  const handleDeactivate = async () => {
    ...
    try {
      const data = await apiFetch<{ tenant: TenantDetail }>(`/api/v1/admin/tenants/${params.id}/deactivate`, {
        method: "POST",
      });
      setTenant(data.tenant);
    } catch (err) {
      ...
      <div className="rounded-lg bg-white p-6 shadow">
        ...
        <div className="mb-6 grid grid-cols-2 gap-4">
          <div className="rounded-md bg-gray-50 p-4">
            ...
            <p className="text-2xl font-bold">{tenant.user_count} / {tenant.max_users}</p>
          </div>
          <div className="rounded-md bg-gray-50 p-4">
          ...
          <div className="mb-6 space-y-4 rounded-md border p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={form.max_documents} onChange={(e) => setForm({ ...form, max_documents: +e.target.value })}
                  className="mt-1 block w-full rounded-md border px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm text-gray-700">Max Storage (GB)</label>
                <input type="number" value={form.max_storage_gb} onChange={(e) => setForm({ ...form, max_storage_gb: +e.target.value })}
                  className="mt-1 block w-full rounded-md border px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm text-gray-700">Max Model Versions</label>
                <input type="number" value={form.max_model_versions} onChange={(e) => setForm({ ...form, max_model_versions: +e.target.value })}
                  className="mt-1 block w-full rounded-md border px-3 py-2 text-sm" />
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={handleUpdate} className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">Save</button>
              <button onClick={() => setEditing(false)} className="rounded-md bg-gray-200 px-4 py-2 text-sm hover:bg-gray-300">Cancel</button>
            </div>
          </div>
        ) : (
          <div className="mb-6 flex gap-3">
            <button onClick={() => setEditing(true)} className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">
              Edit Quotas
            </button>
            {tenant.status === "active" && (
              <button onClick={handleDeactivate} className="rounded-md bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-700">
                Deactivate Tenant
              </button>
            )}
          </div>
