...
<!-- TBD: This spec covers the role-specific SQL query wiring for the dashboard summary endpoint. Each role (tenant_admin, annotator, business_user) has dedicated query handlers that execute real SQL against the tenant's schema tables. Failed queries degrade gracefully rather than failing the entire request. -->
