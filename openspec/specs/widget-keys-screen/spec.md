# Spec: Widget Keys Screen

## Purpose

Defines the Widget Keys screen at `/widget-keys`, accessible only to `tenant_admin` users, for viewing embeddable widget API keys backed by the existing `/api/v1/tenants/{slug}/widget-keys` endpoint.
