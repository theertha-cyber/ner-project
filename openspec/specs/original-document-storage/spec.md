...
Defines the content-store boundary (put, open, delete), the three retention modes
and the content resolution each implies, the bounded working copy that makes
ephemeral retention work, bounded reprocessability by retention mode, and the
prohibition on any consumer parsing the storage reference.
