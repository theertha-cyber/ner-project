...
The system SHALL generate suggested entity spans for a document using the tenant's `base_label_mapping` configuration. Suggested spans SHALL be stored in a separate `suggested_spans` table with confidence scores. Pre-labeling SHALL replace all existing suggested spans for the document with newly generated ones. For MVP, the pre-labeling logic SHALL be a deterministic mock — it SHALL scan document text for matches against label keywords derived from the tenant's label mapping.

#### Scenario: Pre-label a processed document

- **GIVEN** a processed document with text "Acme Corp hired John Doe" and a tenant with `base_label_mapping: {"PER": ["John Doe"], "ORG": ["Acme Corp"]}`
- **WHEN** an annotator POSTs to `/api/v1/documents/{doc_id}/prelabel`
- **THEN** the response SHALL have status 200
- **AND** the response body SHALL contain suggested spans for "John Doe" (PER) and "Acme Corp" (ORG)
- **AND** each suggested span SHALL have `confidence < 1.0`
...
- **GIVEN** a suggested span with ID "suggest-1" for entity type "PER" at offsets 10-18
