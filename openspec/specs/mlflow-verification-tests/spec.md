...
The test script SHALL verify that the Model Registry proxy functions work correctly against a live MLflow server.
...
The test script SHALL verify that the Model Registry proxy falls back to the local PostgreSQL cache when MLflow is unreachable.
