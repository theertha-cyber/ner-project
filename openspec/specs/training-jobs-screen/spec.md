...
Frontend specification for the Training Jobs screen (`/training-jobs`), covering the job list, filter tabs, status timeline, dataset-to-model lineage diagram, live running-job callout, evaluation metrics display, MLflow run link card, default-selection behavior, and the submit job slide-over. See `openspec/specs/training-jobs/spec.md` for the backend API spec.
