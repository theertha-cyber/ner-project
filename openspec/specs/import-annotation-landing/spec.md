...
annotations" (linking to plain `/imported-documents` — the file picker there opens only on an
explicit click, not on arrival) and "2. Train model" (linking to `/training-jobs?source=import`,
the Import workflow's own hand-off, so a run started from here trains only on imported data).
Step 2 SHALL be shown regardless of whether any file is yet training-eligible — imported data
skips labeling and goes straight to training-eligible once resolved, so there is no separate
review step gating access to training. For `annotator`, the page SHALL render exactly one
"Where the work is" card, "Imported files", linking to `/imported-documents` — an annotator does
not import files or train models, only reviews them, so neither workflow step SHALL be shown to
that role.
...
- **THEN** exactly two cards SHALL be present, in order: "1. Import annotations", "2. Train
  model"
- **AND** activating "1. Import annotations" SHALL navigate to `/imported-documents`
