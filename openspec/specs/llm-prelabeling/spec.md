...
LLM-driven span suggestion generation for a document, as an alternative source alongside the keyword matcher described in `annotation-workspace`. Covers the trigger endpoint, extractive-only prompting constrained to the tenant's configured entity types, verbatim-quote grounding against document text, asynchronous execution, and content-hash-based caching. Every suggestion it produces still passes through the existing human-approval gate before becoming a confirmed span.
