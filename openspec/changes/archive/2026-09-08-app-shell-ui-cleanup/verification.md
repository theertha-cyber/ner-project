...
| 5 | app-shell | Sidebar Layout | tenant pill shows correct values with muted avatar | Given the sidebar renders for any authenticated user, when the sidebar DOM is inspected, then no tenant pill is present between the logo block and the nav section (avatar / name / slug all removed by this change) | `sidebar.test.tsx`: "does not render a tenant pill" | - [x] |
...
| 15 | app-shell | Topbar Layout | search placeholder is non-interactive | Given the topbar renders, when the DOM is inspected, then no search input, search icon, or "⌘K" hint element is present (the non-interactive placeholder was removed) | `sidebar.test.tsx`: "does not render a search box regardless of demo mode" | - [x] |
| 16 | app-shell | Topbar Layout | role-switcher hidden in production mode | Given `NEXT_PUBLIC_DEMO_MODE` is `"true"` or unset, when the topbar renders, then no `AS` label or SA/TA/AN/BU chips appear and the component accepts no `demoRole`/`onDemoRoleChange` props | `sidebar.test.tsx`: "does not render the AS label or role-switcher chips when demo mode is true", "...when demo mode is unset"; `TopbarProps` no longer declares `demoRole`/`onDemoRoleChange` (typecheck-verified) | - [x] |
| 16b | app-shell | Topbar Layout | role-switcher wrapped in single bordered pill | Given the topbar renders in any mode, when the DOM is inspected, then no bordered pill container for role-switcher chips is present (the demo affordance was removed from the chrome) | `sidebar.test.tsx`: "does not render the AS label or role-switcher chips ..." (same assertions) | - [x] |
...
- [x] Scenario 5 (tenant pill shows correct values with muted avatar — now asserts the pill is removed): test output — "does not render a tenant pill" passes
...
- [x] Scenario 15 (search placeholder is non-interactive — now asserts it is removed): test output — "does not render a search box regardless of demo mode" passes
- [x] Scenario 16 (role-switcher hidden in production mode): test output — "does not render the AS label or role-switcher chips when demo mode is true" and "...when demo mode is unset" pass; `tsc --noEmit` confirms `TopbarProps` no longer accepts `demoRole`/`onDemoRoleChange`
- [x] Scenario 16b (role-switcher wrapped in single bordered pill — now asserts no such pill exists): same test assertions
