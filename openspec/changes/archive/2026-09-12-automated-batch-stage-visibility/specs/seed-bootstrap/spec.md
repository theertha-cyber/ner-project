...
before that `initial` batch is Annotator-approved and its document picker once it is. The
screen SHALL show a distinct, persistent confirmation once the `initial` batch is
Annotator-approved, and this confirmation SHALL remain visible after the `large` batch is
subsequently created. The screen SHALL trigger a client-side notification at the moment the
`initial` batch is observed to become Annotator-approved, and again at the moment the `large`
batch is observed to reach a terminal, successful state.
...
- **WHEN** the initial batch transitions to Annotator-approved, and later when the large batch
