...
Before the document loop, and exactly once per run, the worker SHALL load the tenant's entity definitions and reconcile the tenant schema's generated relational tables in that reconciliation's **own** transaction. Reconciliation SHALL NOT occur inside a per-document transaction and SHALL NOT occur per document. For each successfully processed document, the worker SHALL — inside the single existing per-document transaction — delete that document's `document_entities` rows, delete that document's rows from every generated relational table, insert `extracted_entities`, insert `document_entities`, and project the final in-memory entity list into the generated relational tables. `extracted_entities` SHALL NOT be deleted by the worker, because it is the idempotency ledger that determines which documents are skipped.
...
#### Scenario: The run reconciles the relational schema once before processing

- **GIVEN** a tenant whose schema has no generated entity tables and two active entity definitions
- **WHEN** a batch extraction run starts
- **THEN** both definitions' generated tables SHALL exist before the first document is processed
- **AND** reconciliation SHALL have run exactly once for the run, not once per document

#### Scenario: A freshly provisioned tenant extracts successfully on its first run

- **GIVEN** a tenant provisioned by cloning `tenant_template`, which carries no generated entity tables
- **WHEN** its first batch extraction run executes
- **THEN** the run SHALL create the required tables at run start
- **AND** documents SHALL be projected without failure

#### Scenario: Both stores are written in one transaction per document

- **GIVEN** a document that extracts entities routed to active definitions
- **WHEN** the document's transaction commits
- **THEN** `document_entities` and the generated relational tables SHALL both reflect that document's final entity list
- **AND** no write to either store SHALL occur outside that transaction

#### Scenario: A document failure leaves both stores clean and the run continues

- **GIVEN** a batch run over three documents where the second raises during persistence
- **WHEN** the run completes
- **THEN** `processed_count` SHALL be 2 and `failed_count` SHALL be 1
- **AND** the failed document SHALL have no rows in `document_entities` or any generated table
- **AND** the third document SHALL still be processed

#### Scenario: Re-extraction under a new model version replaces rather than appends

- **GIVEN** a document already extracted under model version 3 with five entities persisted in `document_entities`
- **WHEN** a batch run under model version 4 re-extracts it and produces five entities
- **THEN** `document_entities` SHALL contain five rows for that document, not ten
- **AND** the generated relational tables SHALL contain only the newest extraction's rows
- **AND** `extracted_entities` SHALL retain both runs' rows

#### Scenario: Reconciliation emits no DDL inside a document transaction

- **GIVEN** any batch extraction run
- **WHEN** a per-document transaction executes
- **THEN** it SHALL contain no `CREATE`, `ALTER`, or `DROP` statement
