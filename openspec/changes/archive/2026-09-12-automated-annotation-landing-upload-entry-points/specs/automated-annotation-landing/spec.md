...
above the primary "Start with step 1" action: "Upload documents" (linking to
`/documents?upload=1&purpose=training&mode=automated`) and "Upload Q&A pair" (linking to
`/documents?upload=1&purpose=qa_pair`). These SHALL be visible regardless of stepper state, since
Step 1 ("Suggest Entity Types") depends on seed documents already sitting in the tenant's
document library that this page itself provides no other path to add.
