...
> into model training. It is the first screen a Tenant Admin or Annotator sees when they
> choose manual annotation over the automated or import flows.
...
### Requirement: Single Work Card

The page SHALL render exactly one "Where the work is" card, "Annotation workspace", linking
to `/annotation`. The page SHALL NOT render a "Review queue" card or any other work card.

#### Scenario: only the workspace card is shown

- **GIVEN** `/annotate/manual` renders for any role
...
navigates to `/training-jobs` when activated. The action SHALL NOT be rendered when zero
tasks are completed, and SHALL NOT be rendered for `annotator`.
...
- **THEN** a "Train model" action SHALL be visible reading "99 documents annotated and ready
  for training"
- **AND** activating it SHALL navigate to `/training-jobs`
