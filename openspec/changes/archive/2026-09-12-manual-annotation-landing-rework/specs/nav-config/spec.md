...
`Manual` SHALL carry child `Workspace /annotation`. `Automated` SHALL carry children for the
four step routes `/annotate/automated/schema`, `/annotate/automated/prelabel`,
...
- **AND** no child with href `/review-queue` SHALL be present
