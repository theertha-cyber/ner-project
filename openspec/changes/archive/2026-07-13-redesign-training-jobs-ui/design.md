...
### Decision 4: Lineage's DATASET box is a static/decorative label, not a per-job field

**Choice:** The lineage diagram's first box always reads a static label ("Annotated Documents") rather than a per-job dataset name. The middle box uses `job.id`; the third box resolves the promoted model version's `version_number` by matching `training_job_id === job.id` against the existing `/api/v1/models` list (via `useModelVersions`), falling back to "pending" when no match exists.

**Rationale:** Implementation discovered that no dataset entity exists anywhere in this system — `TrainingJob` (frontend type, `TrainingJobResponse` schema, and the SQLAlchemy model) has no dataset name/id field, and training jobs consume a tenant's entire confirmed-annotation export rather than a named/versioned dataset. Inventing a `dataset` field would require a backend/schema change, which is out of scope for this presentation-only change and violates the Non-Goals. The model-version side of the lineage, however, is real data already exposed by the existing `/api/v1/models` endpoint (`version_number`, `training_job_id`), so no placeholder is needed there.

**Alternatives considered:**
- Add a real `dataset` field to `TrainingJob` — rejected, requires backend/schema work outside this change's scope.
- Drop the dataset box entirely (2-box lineage) — rejected in favor of keeping the mockup's 3-box visual, matching how the GPU-worker placeholder in `JobProgress` is already handled (decorative text, no real field).

### Decision 5: Metrics keys are `eval_*`-prefixed, matching what the training worker actually persists

**Choice:** `JobCard`'s F1 value and `JobMetrics`' stat group read `metrics.eval_f1`, `metrics.eval_precision`, `metrics.eval_recall`, and `metrics.eval_loss` — not bare `f1`/`precision`/`recall`/`loss` as the proposal's original illustrative examples used. Displayed labels stay short ("f1", "precision", "recall", "eval_loss") since the label is a presentation choice independent of the underlying key name.

**Rationale:** `src/training_service/worker.py` (`compute_metrics` and the final `metrics` dict written to the job row) always persists these four values under the `eval_`-prefixed keys — that prefix comes directly from HuggingFace `Trainer.evaluate()`'s output convention. Bare keys never appear in real data; had the components read `metrics.f1` etc., the F1 column and the metrics stat group would silently render empty/"—" for every real job, which is the "AI-invented field" hallucination risk this change's own register (item 4) warns about, just on the metrics side rather than GPU-worker.

**Alternatives considered:**
- Keep bare keys as scenario text and normalize/rename in the API layer — rejected, would be a backend change outside this change's presentation-only scope for no benefit over reading the existing key names directly.
