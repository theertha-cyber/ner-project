...
- **WHEN** POST is sent to `/api/v1/tenants/{tid}/widget-keys`
...
- **WHEN** a Tenant Admin GETs `/api/v1/tenants/{tid}/widget-keys`
- **THEN** the response SHALL have status 200
...
- **WHEN** a Tenant Admin sends DELETE to `/api/v1/tenants/{tid}/widget-keys/{key_id}`
