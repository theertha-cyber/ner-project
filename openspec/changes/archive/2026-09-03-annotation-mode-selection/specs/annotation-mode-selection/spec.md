...
The system SHALL render an annotation-mode selector with exactly two options, "Manual" and "Automated", within the document upload flow when `purpose` is `"training"`. The system SHALL NOT render the selector when `purpose` is `"query"`. The selector SHALL default to "Manual" on every batch, and SHALL reset to "Manual" after a batch completes.
