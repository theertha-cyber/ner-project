...
Defines the local delivery contract for the tenant self-service data-source feature (CAP-2 through CAP-5) inside the approved single `dev` Docker Compose environment: ordered compatible delivery, health/readiness-gated replacement, safe operational telemetry, and a documented compatible recovery path. Staging, production, and all production operating targets are explicitly out of scope.
