...
| 12 | document-ingestion-boundary | Platform upload is an adapter over the ingestion contract | Upload behaviour is unchanged | Given the existing document ingestion test suite, when run against the refactored route with no test modified, then every test passes | existing `tests/test_document_ingestion.py` + `tests/test_document_content_hash.py`, unmodified, plus clean `git diff` (task 3.9) | - [ ] |
...
- [ ] Row 12: full run of `tests/test_document_ingestion.py` and `tests/test_document_content_hash.py` with a clean `git diff` proving no test file was modified
