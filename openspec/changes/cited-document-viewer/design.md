...
- **In-request conversion or a job?** The assumption here is in-request with a bounded timeout, on the grounds that a job adds a state machine and a polling contract for a case measured in seconds. If large `.docx` conversions prove slow in practice this should become a job, and the probe is the natural place to report its progress.
- **May a rendition be cached for an `ephemeral` document?** The assumption is no (Decision 7). Confirm before implementation: it is the one place this change could quietly undo a tenant's retention choice.
