...
- [ ] No backend files modified — `git diff --stat` shows changes confined to `src/portal/`

### Edge Case Evidence

*(One item per Hallucination Risk from Section 2.)*

- [ ] Risk 1 mitigation confirmed — trigger loop is a separate phase after the upload loop, not interleaved
- [ ] Risk 2 mitigation confirmed — no `Promise.all`/`allSettled` in the trigger path; sequential awaits only
- [ ] Risk 3 mitigation confirmed — gating predicate tests active entity type count only, never `qa_examples` presence
- [ ] Risk 4 mitigation confirmed — trigger loop never writes upload item status and has no early exit on error
- [ ] Risk 5 mitigation confirmed — every new call site is behind a mode check; Manual path verified unchanged against the real component
- [ ] Risk 6 mitigation confirmed — selector render condition reads the `purpose` prop, not a role check
- [ ] Risk 7 mitigation confirmed — trigger call routes through `auth-fetch.ts`; no raw `fetch(` in new code
