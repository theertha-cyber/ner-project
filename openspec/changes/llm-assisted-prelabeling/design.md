...
- Which LLM provider/model is the default, and is it configurable per-tenant? (Blocks implementation, not spec review.)
...
- Duplicate-quote resolution (a quote matching multiple locations in the text): this design assumes "first occurrence not already covered by another suggested span," mirroring the keyword matcher's overlap rule — flagged for explicit reviewer sign-off since it directly affects recall and hasn't been validated against real documents.
