...
async def _run_tenant_schema_ddl(conn, ddl_template: str):
    await conn.execute(text(ddl_template.format(schema="tenant_template")))
    result = await conn.execute(text("SELECT id FROM public.tenants"))
    for row in result:
        tid = row[0]
        schema = f"tenant_{tid.replace('-', '_')}"
        await conn.execute(text(ddl_template.format(schema=schema)))
        ...
        try:
            async with engine.begin() as conn:
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_template.test_table "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
                await conn.execute(text("CREATE SCHEMA IF NOT EXISTS tenant_test_tenant"))
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_test_tenant.test_table "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
            async with engine.begin() as conn:
                await _run_tenant_schema_ddl(
                    conn,
                    "ALTER TABLE IF EXISTS {schema}.test_table ADD COLUMN IF NOT EXISTS foo VARCHAR",
                )
            async with engine.begin() as conn:
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DROP TABLE IF EXISTS tenant_test_tenant.test_table CASCADE"))
                await conn.execute(text("DROP TABLE IF EXISTS tenant_template.test_table CASCADE"))
    ...
    async def test_inactive_tenant_schema_still_updated(self, engine, setup_database):
        try:
            async with engine.begin() as conn:
                ...
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_inactive_tenant.test_table "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
            async with engine.begin() as conn:
                await _run_tenant_schema_ddl(
                    conn,
                    "ALTER TABLE IF EXISTS {schema}.test_table ADD COLUMN IF NOT EXISTS bar VARCHAR",
                )
            async with engine.begin() as conn:
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DROP SCHEMA IF EXISTS tenant_inactive_tenant CASCADE"))
                await conn.execute(text("DROP TABLE IF EXISTS tenant_template.test_table CASCADE"))
                await conn.execute(text("DELETE FROM public.tenants WHERE id = 'inactive-tenant'"))
    ...
    async def test_re_running_ddl_is_no_op(self, engine, setup_database):
        try:
            async with engine.begin() as conn:
                ...
                await conn.execute(text(
                    "INSERT INTO public.tenants (id, name, slug, status) "
                    "VALUES ('noop-tenant', 'Noop', 'noop-tenant', 'active') "
                    "ON CONFLICT (id) DO NOTHING"
                ))
                await conn.execute(text("CREATE SCHEMA IF NOT EXISTS tenant_noop_tenant"))
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_noop_tenant.test_table "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
            async with engine.begin() as conn:
                await _run_tenant_schema_ddl(
                    conn,
                    "ALTER TABLE IF EXISTS {schema}.test_table ADD COLUMN IF NOT EXISTS baz VARCHAR",
                )
                await _run_tenant_schema_ddl(
                    conn,
                    "ALTER TABLE IF EXISTS {schema}.test_table ADD COLUMN IF NOT EXISTS baz VARCHAR",
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DROP SCHEMA IF EXISTS tenant_noop_tenant CASCADE"))
                await conn.execute(text("DROP TABLE IF EXISTS tenant_template.test_table CASCADE"))
                await conn.execute(text("DELETE FROM public.tenants WHERE id = 'noop-tenant'"))
    ...
    async def test_backfill_adds_error_message_to_template_and_tenants(self, engine, setup_database):
        try:
            async with engine.begin() as conn:
                ...
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_backfill_tenant.training_jobs "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
            async with engine.begin() as conn:
                await _run_tenant_schema_ddl(
                    conn,
                    "ALTER TABLE IF EXISTS {schema}.training_jobs ADD COLUMN IF NOT EXISTS error_message TEXT",
                )
            async with engine.begin() as conn:
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DROP SCHEMA IF EXISTS tenant_backfill_tenant CASCADE"))
    ...
    async def test_already_matching_tenant_unaffected(self, engine, setup_database):
        try:
            async with engine.begin() as conn:
                ...
                await conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS tenant_match_tenant.training_jobs "
                    "(LIKE tenant_template.training_jobs "
                    "INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES)"
                ))
            async with engine.begin() as conn:
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DROP SCHEMA IF EXISTS tenant_match_tenant CASCADE"))
